...
% Last Modified by GUIDE v2.5 31-Jan-2011 15:34:19
%
%   M. Misiti, Y. Misiti, G. Oppenheim, J.M. Poggi 01-Jul-2010.
%   Last Revision 01-Nov-2012.
%   Copyright 1995-2012 The MathWorks, Inc.
%   $Revision: 1.1.6.11 $  $Date: 2012/12/03 01:57:35 $ 
...
switch OPT
    ...
    case 3
        ...
        ScType = getScType(scales);
        % QWARN = questdlg(...
        %     'Are you sure that the manual scales are the good ones?', ...
        %     'Manual Scales for Analysis','Yes', 'No','Yes');
        % switch QWARN,
        %     case 'Yes'
        %     case 'No',  wwaiting('off',fig); return;
        % end
...
Tab_Synt_Status = [hdl_CHK , zeros(3,3)];
...
% ---------
AP.sampPer = 1; 
SCA.s0 = 10*AP.sampPer; SCA.ds = SCA.s0; SCA.nb = 200; 
SCA.type = 'lin';
AP.SCA = SCA;
...
switch option
    case {'load_beg','anal_beg'}
        ...
        switch option
            case 'load_beg'
                ...
                hdl_AXE_Child = allchild(...
                    [hdl_Axe_SIG ; hdl_Axe_ANAL ; axCB ; hdl_Axe_MAN]);
            ...
            case 'anal_beg' 
                ...
                hdl_AXE_Child = allchild(...
                    [hdl_Axe_ANAL ; axCB ;]); % handles.Axe_MAN_SEL
        ...
        Tab_Synt_Status = [CHK_REC , zeros(3,3)];
...
tags = get(axInFig,'Tag');
idxColBar = strcmp(tags,'Colorbar');
axeCB = axInFig(idxColBar);
...
if option>0
    ...
    for j = 1:3
        if Tab_Synt_Status(j,2)
            nb = nb+1;
            errMAX = 100*Tab_Synt_Status(j,3);
            errL2  = 100*Tab_Synt_Status(j,4);
            switch j
                case 1
                    SS = getWavMSG('Wavelet:cwtfttool:RelativeErrorFromInitialScales', ...
                        sprintf('%3.2f',errMAX),sprintf('%3.2f',errL2));
                case 2
                    SS = getWavMSG('Wavelet:cwtfttool:RelativeErrorFromListOfScales', ...
                        sprintf('%3.2f',errMAX),sprintf('%3.2f',errL2));
                case 3
                    SS = getWavMSG('Wavelet:cwtfttool:RelativeErrorFromManualSelection', ...
                        sprintf('%3.2f',errMAX),sprintf('%3.2f',errL2));
            end
            if isempty(LabSTR)
                LabSTR = SS;

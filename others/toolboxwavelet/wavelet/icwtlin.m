...
%   XREC = ICWTLIN(CWTSTRUCT) returns the recontructed signal XREC
%   using the data containted in the structure CWTSTRUCT.
%   CWTSTRUCT may be obtained with a continuous analysis using CWTFT
%   function or may be built with an analysis using CWT.
%
%   In the first case, CWTSTRUCT is a structure which contains
%   six fields:
...
%      dt:      sampling period.
%	(see CWTFTINFO for information on the available wavelets).
%   In the second case, CWTSTRUCT must be a structure containing
%   the five fields required for the inverse transform (see above).
%
%   In last case, the available wavelets are:
...
%      - morl, mexh
%
%   XREC = ICWTLIN(WAV,meanSIG,cfs,scales,dt) returns also the
%   recontructed signal.
%
%   With XREC = ICWTLIN(...,'plot'), the reconstructed signal from
%   the continuous wavelet coefficients is plotted.
%
%   In addition, with XREC = ICWTLIN(...,'signal',SIG,'plot') the
%   original signal is superimposed.
%
%   XREC = ICWTFT(...,'IdxSc',IdxSc2Inv) returns the reconstructed
%   signal obtained by keeping the coefficients associated to the
%   scales which indices are given by the vector IdxSc2Inv.
%
%   CAUTION: To get a significant reconstructed signal, the scales
...
%   $Revision: 1.1.6.6 $  $Date: 2012/06/03 06:27:10 $
...
if isstruct(varargin{1})
    ...
    dt      = CWTS.dt;
    nextARG = 2;
    if isfield(CWTS,'omega') , recTYPE = 'cwtft'; else recTYPE = 'cwt'; end
else

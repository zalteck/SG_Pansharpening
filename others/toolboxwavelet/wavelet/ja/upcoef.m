...
%
% Y = UPCOEF(O,X,'wname',N) は、ベクトル X の N ステップ目の再構成された
% 係数を計算します。'wname' は、ウェーブレット名を含む文字引数です。
% N は、厳密な意味での正の整数です。
% O = 'a' の場合、Approximation 係数が再構成されます。
% O = 'd' の場合、Detail 係数が再構成されます。
%
% Y = UPCOEF(O,X,'wname',N,L) は、ベクトル X の N ステップの再構成された
% 係数を計算します。そして、結果の中から中央の L の長さを抽出します。
%

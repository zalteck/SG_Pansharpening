...
%   $Revision: 1.13.4.11 $ $Date: 2012/03/01 03:09:27 $
...
switch option
    case 'ini'
        ...
        figName = getWavMSG('Wavelet:divGUIRF:DynV_ViewAxes',num2str(fig));
        ...
        try
            uicCOLM = utcolmap('handles',fig);
        catch ME  %#ok<NASGU>
            uicCOLM = [];
        end
        a   = wcommon(uic,[closeBTN;uicDYNV(:);uicCOLM(:)]);
        uic = uic(~a);

...
%   $Revision: 1.1.6.8 $ $Date: 2012/07/05 17:26:53 $ 
...
switch nbIN
    ...
    case 2  % Multisignal Clustering Tool
        ...
        Data_Name = [mfilename '_' int2str(callingFIG) '_DATA.mat'];
        save([pwd filesep Data_Name],'signals');
...
wtbxappdata('set',fig,'filedataname',Data_Name);
...
InVIS = [...
    handles.Rad_AFF_SIG,handles.Rad_AFF_DAT,  ...
    handles.Fra_PSEL_VIS,handles.Txt_PSEL_VIS ...
    ];
...
%--------------------------------------------------------------------------
function Lst_SIM_IDX_Callback(hObject,eventdata,handles) %#ok<INUSD,DEFNU>
end
%--------------------------------------------------------------------------
...
sep = '  =  ';
strINDEXS = [...
    strvcat(FN{:}) , repmat(sep,length(strVAL),1) , strvcat(strVAL{:})]; 
set(handles.Lst_SIM_IDX,'String',strINDEXS,'Value',[]);
end
%--------------------------------------------------------------------------
function Pus_VAL_MOD_Callback(hObject,eventdata,handles) %#ok<INUSL,DEFNU>
...
%--------------------------------------------------------------------------
function Pus_CON_PART_Callback(hObject,eventdata,handles) %#ok<INUSL,DEFNU>
...
%--------------------------------------------------------------------------
function Pus_CON_EXE_Callback(hObject,eventdata,handles) %#ok<INUSL,DEFNU>
...
%--------------------------------------------------------------------------
function Pus_CON_SAV_Callback(hObject,eventdata,handles,flagSAVE) %#ok<INUSL,DEFNU>
...
%--------------------------------------------------------------------------
function Pop_Nb_CON_Callback(hObject,eventdata,handles) %#ok<INUSD,DEFNU>
end
%----------------------------------------------------------
function Pop_CON_MTH_Callback(hObject,eventdata,handles) %#ok<INUSD,DEFNU>
end
%----------------------------------------------------------

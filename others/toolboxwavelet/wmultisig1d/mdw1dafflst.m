...
%   Last Revision 24-Jun-2012.
%   Copyright 1995-2012 The MathWorks, Inc.
%   $Revision: 1.1.6.11 $ $Date: 2012/08/01 01:16:32 $
...
%--------------------------------------------------------------------------
    function aff_LST_ORI
            ...
            if flagSIG
                for k = 1:length(num_typeSIG)
                    ...
                    elseif num_typeSIG(k)==1 || num_typeSIG(k)==2
                        ...
                        Energy  = [Energy  ; L2_DorC]; %#ok<AGROW>
                        tab_ENER = [tab_ENER ; tab_ENER_DorC]; %#ok<AGROW>
                    end
                end
        ...
        STR_VAL = blanc;
        for k = 1:lastINFO-1
            STR_VAL = [STR_VAL  , num2str(tab_INFO(:,k),intFormat) , sep]; %#ok<AGROW>
        end
        ...
        for k = first+3:last
            if k~=col_SORT-1
                STR_VAL = [STR_VAL , num2str(tab_INFO(:,k),formatNum) , sep]; %#ok<AGROW>
            else
                STR_VAL = [STR_VAL , num2str(tab_INFO(:,k),formatNum) , star]; %#ok<AGROW>
            end
        end
        if nbINFO>0
            for k = last+1:size(tab_INFO,2)-1
                if k~=col_SORT-1
                    STR_VAL = [STR_VAL , num2str(tab_INFO(:,k),formatPER) , sep]; %#ok<AGROW>
                else
                    STR_VAL = [STR_VAL , num2str(tab_INFO(:,k),formatPER) , star]; %#ok<AGROW>
                end
            end
            formatNum_Ener = '%0.4g';
...
%--------------------------------------------------------------------------
    function aff_LST_CMP(varargin)
        ...
        switch aff_OPT
            case {'init','importCLU'}
                ...
                for k=1:level
                    ...
                    str_DATA = [str_DATA , [' ' strTHR ' |']]; %#ok<AGROW>
                end
        ...
        for k = nbINFO-1:nbINFO
            if k~=col_SORT-1
                STR_VAL = ...
                    [STR_VAL , num2str(tab_INFO(:,k),formatPER) , sep]; %#ok<AGROW>
            else
                STR_VAL = ...
                    [STR_VAL, num2str(tab_INFO(:,k),formatPER) , star]; %#ok<AGROW>
            end
        end
        if nbSigInSEL==1 , STR_VAL = {STR_VAL}; end
...
%--------------------------------------------------------------------------
    function aff_LST_CLU(varargin)
        ...
        for k=1:nb_COL_Ts
            STR_VAL = [STR_VAL,num2str(IdxCLU(idxSORT,k),'%2.0f'),B1, sep]; %#ok<AGROW>
        end
        ...
        switch aff_OPT
            ...
            case 'clu_SEL'
                ...
                nbSEL = 2;
                for k=1:nbSEL
                    str_DATA = [str_DATA  ' P' int2str(k) ' |']; %#ok<AGROW>
...
%--------------------------------------------------------------------------
    function StrOUT = Make_ToolTipString(StrIN)
        ...
        for k=1:length(StrIN)
            StrOUT = [StrOUT , StrIN{k} , '   |   ']; %#ok<AGROW>
        end
    end
%--------------------------------------------------------------------------
